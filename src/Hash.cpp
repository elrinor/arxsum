...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string finalize() {
			...
			std::string result;
			// Presuming little endian data order
			for(int32 i = 3; i >= 0; i--)
				result += hexDump(((byte*) &this->crc32)[i]);
			return result;
		}
...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string MD4::finalize() {
			...
			return hexDump(finalizeBinary(md), MD4_DIGEST_LENGTH);
		}

		unsigned char* MD4::finalizeBinary(unsigned char* md) {
	...
	public:
		...
		void update(const void* data, size_t len) {
			...
			while(len > 0) {
				...
				if(this->blockPos == 0) {
					finMD4.update((const void*) mainMD4.finalizeBinary(md), MD4_DIGEST_LENGTH);
					mainMD4.init();
				}
			}
		}

		std::string ED2K::finalize() {
			// Elric [1/14/2007]:
			...
			if(this->pos >= ed2kBorder) {
				unsigned char md[MD4_DIGEST_LENGTH];
				this->finMD4.update((const void*) this->mainMD4.finalizeBinary(md), MD4_DIGEST_LENGTH);
...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string finalize() {
			...
			return hexDump(md, MD5_DIGEST_LENGTH);
		}
...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string finalize() {
			...
			return hexDump(md, SHA_DIGEST_LENGTH);
		}
...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string finalize() {
			...
			return hexDump(md, SHA256_DIGEST_LENGTH);
		}
...
// ------------------------------------------------------------------------- //
	...
	public:
		...
		std::string finalize() {
			...
			return hexDump(md, SHA512_DIGEST_LENGTH);
		}
...
Hash::Hash(uint32 hashId, uint64 totalLen): hashId(hashId) {
	switch(hashId) {
	case H_CRC:
		impl.reset(new __hashes::CRC());
		break;
	case H_MD4:
		impl.reset(new __hashes::MD4());
		break;
	case H_ED2K:
		impl.reset(new __hashes::ED2K());
		break;
	case H_MD5:
		impl.reset(new __hashes::MD5());
		break;
	case H_SHA1:
		impl.reset(new __hashes::SHA1());
		break;
	case H_SHA256:
		impl.reset(new __hashes::SHA256());
		break;
	case H_SHA512:
		impl.reset(new __hashes::SHA512());
		break;
	case H_TTH:
		impl.reset(new __hashes::TTH(totalLen));
		break;
	default:
