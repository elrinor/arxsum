...
int CRC_Final(unsigned char *md, CRC_CTX *c)
{
	...
	md[0] = ((unsigned char*)c)[3];
	md[1] = ((unsigned char*)c)[2];
	md[2] = ((unsigned char*)c)[1];
	md[3] = ((unsigned char*)c)[0];
	return 1;
}
...
{
	CRC_CTX c;
	static unsigned char m[CRC_DIGEST_LENGTH];
