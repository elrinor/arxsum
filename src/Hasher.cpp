...
namespace __hasher {
	...
	public:
		...
		void hash(FileEntry entry) {
			ChunkInputStream stream = createChunkInputStream(entry.getPath());
			...
			while(!stream.eof()) {
				...
				uint32 read = stream.read(buf.c_array(), buf.size());
				totalRead += read;
				doCallBack(read);
	...
	public:
		...
		void hash(FileEntry entry) {
			...
			while(!stream.eof()) {
				...
				uint32 read = stream.read(buf0, bufSize);
				totalRead += read;
				doCallBack(read);
...
Hasher::Hasher(HashTask task, bool isMultiThreaded, shared_ptr<HasherCallBack> callBack) {
	if(isMultiThreaded)
		impl.reset(new __hasher::MultiThreadedHasher(task, callBack));
	else
		impl.reset(new __hasher::SimpleHasher(task, callBack));
}
...
string Hasher::hash(uint32 hashId, path filePath, shared_ptr<HasherCallBack> callBack) {
	HashTask task;
	task.add(hashId);
	__hasher::SimpleHasher hasher(task, callBack);
...
void StreamOutCallBack::operator() (uint64 justProcessed) {
	...
	*writer << "\rProgress: " << setw(3) << progress << "%";
	...
	if(currentTime - lastOutputTime > OUTPUTPERIOD) {
		...
		if(speed != 0.0)
			*writer << "   Speed: " << fixed << showpoint << setprecision(3) << speed / (1024 * 1024) << "Mbps       ";
	}
}

void StreamOutCallBack::clear() {
	*writer << "\r                                        \r";
}
